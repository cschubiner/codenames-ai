...
### 4a) Cheap smoke run (recommended first)

The default configs can be expensive (K×G rollouts per turn). Start with these low‑cost configs:

```bash
python scripts/run_match.py \
  --boards data/boards_dev.jsonl \
  --red configs/agent_a_smoke.json \
  --blue configs/agent_b_smoke.json \
  --replicates 1 \
