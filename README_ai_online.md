...
### Human spymasters and controller

The join UI now supports roles:

- **GUESSER** (default for RED/BLUE links): clicks cards for that team.
- **SPYMASTER**: one per team. When a human spymaster is present for the active team, the AI will *not* generate clues; the spymaster must submit a clue in the UI. Spymasters see the full key.
- **CONTROLLER**: can click guesses / “AI play next guess” for either team. Useful for AI vs AI games or when only one guessing team joins.
- **SPECTATOR**: read‑only TV/audience view.
