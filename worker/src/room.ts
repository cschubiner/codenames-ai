...
import type { GameStateInternal, GameStatePublic, Player, Team, PlayerRole } from "./types";
...
interface ClueBody extends AuthedBody {
  clue: string;
  number: number;
}

interface KeyBody extends AuthedBody {}
...
export class CodenamesRoom {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    try {
      ...
      if (path === "/clue" && request.method === "POST") {
        const body = (await request.json()) as ClueBody;
        const res = await this.handleSetClue(game, body);
        return jsonResponse(res);
      }

      if (path === "/key" && request.method === "POST") {
        const body = (await request.json()) as KeyBody;
        const res = await this.handleGetKey(game, body);
        return jsonResponse(res);
      }
  ...
  private toPublic(game: GameStateInternal): GameStatePublic {
    return {
      ...
      players: game.players.map(p => ({ id: p.id, name: p.name, team: p.team, role: p.role, joined_at: p.joined_at })),
  ...
  private async handleJoin(game: GameStateInternal, body: JoinBody): Promise<any> {
    ...
    let role = safeUpper((body as any).role || "") as PlayerRole;
    if (!role) {
      role = team === "SPECTATOR" ? "SPECTATOR" : "GUESSER";
    }
    const allowedRoles: PlayerRole[] = ["GUESSER", "SPYMASTER", "CONTROLLER", "SPECTATOR"];
    if (!allowedRoles.includes(role)) throw new Error("bad_role");

    if (team === "SPECTATOR") {
      // Spectator team can only be SPECTATOR or CONTROLLER role.
      if (role !== "SPECTATOR" && role !== "CONTROLLER") role = "SPECTATOR";
    } else {
      // Team members default to GUESSER unless explicitly SPYMASTER/CONTROLLER.
      if (role === "SPECTATOR") role = "GUESSER";
      if (role === "SPYMASTER") {
        const already = game.players.find(p => p.team === team && p.role === "SPYMASTER");
        if (already) throw new Error("spymaster_taken");
      }
    }
    ...
    const p: Player = { id: playerId, token, name, team, role, joined_at: nowMs() };
    game.players.push(p);
    game.history.push({ t: "player_joined", at: nowMs(), player_id: playerId, name, team, role });
    await this.save(game);

    return { player_id: playerId, token, team, role, name };
  }
  ...
  private async handleGuess(game: GameStateInternal, body: GuessBody): Promise<GameStatePublic> {
    ...
    const isController = p.role === "CONTROLLER";
    if (!isController && p.team !== game.turn) throw new Error("not_your_turn");
  ...
  private async handleAIGuess(game: GameStateInternal, body: AuthedBody): Promise<any> {
    ...
    return {
      can_play: p.role === "CONTROLLER" || p.team === activeTeam,
  ...
  private async kickoffClueGenerationIfNeeded(game: GameStateInternal): Promise<void> {
    ...
    // If a human spymaster is present for the active team, wait for their clue.
    if (this.hasHumanSpymaster(game, game.turn)) return;
  ...
  private async generateAndStoreClue(team: Team, startVersion: number): Promise<void> {
    try {
      ...
      if (this.hasHumanSpymaster(before, team)) return;
  ...
  private hasHumanSpymaster(game: GameStateInternal, team: Team): boolean {
    return game.players.some(p => p.team === team && p.role === "SPYMASTER");
  }

  private async handleSetClue(game: GameStateInternal, body: ClueBody): Promise<GameStatePublic> {
    const p = this.auth(game, body);
    if (game.ended) return this.toPublic(game);
    if (p.role !== "SPYMASTER") throw new Error("not_spymaster");
    if (p.team !== game.turn) throw new Error("not_your_turn");
    if (game.clue.status !== "pending") throw new Error("clue_not_pending");

    const clue = safeUpper(body.clue || "");
    const legality = checkClueLegality(clue, game.board_words);
    if (!legality.ok) throw new Error(`illegal_clue:${legality.reason || "unknown"}`);

    const n = clamp(Math.floor(body.number || 1), 1, 9);
    game.clue = { status: "ready", team: game.turn, clue, number: n, generated_at: nowMs() };
    game.guesses_made_this_turn = 0;
    game.max_guesses_this_turn = clamp(n + 1, 0, 10);
    game.history.push({ t: "clue_ready", at: nowMs(), team: game.turn, clue, number: n });

    await this.save(game);
    return this.toPublic(game);
  }

  private async handleGetKey(game: GameStateInternal, body: KeyBody): Promise<any> {
    const p = this.auth(game, body);
    if (p.role !== "SPYMASTER") throw new Error("not_spymaster");
    if (p.team === "SPECTATOR") throw new Error("not_on_team");
    return { key: game.key, team: p.team, room_id: game.room_id };
  }
}
